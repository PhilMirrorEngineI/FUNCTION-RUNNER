...
MEMORY_API_KEY  = os.getenv("MEMORY_API_KEY")    # same secret your web/editor uses
SAVE_REPLIES    = os.getenv("SAVE_REPLIES", "true").lower() == "true"

# Dave/PMEi identity — SHORT and CORRECT
SYSTEM_IDENTITY = (
    "You are Dave, the assistant for PMEi."
    " PMEi = PhilMirrorEngineI — a lawful-reflection framework that coordinates multiple AI nodes"
    " through symbolic recursion. Tagline: 'Taking the Artificial out of AI.'"
    " Tone: concise, helpful, proactive. Prefer concrete steps and short summaries."
    " You can: generate images (gpt-image-1), run simple web search (Tavily), and create PDFs."
)
...
def parse_kv_command(s: str) -> dict:
    ...
    Supports single quotes: content='hello world'
    """
    out = {}
    buf = ""
    in_quote = False
...
# allow Editor “Actions” style ops via /chat
def proxy_get_memory(params: dict) -> dict:
    ...
    try:
        ...
        if not r.ok:
            return {"ok": False, "error": f"Getter HTTP {r.status_code}", "code": r.status_code}
        data = r.json()
        return {"ok": True, "source": "memory_api", **data}
...
def proxy_save_memory(params: dict) -> dict:
    ...
    try:
        ...
        if not r.ok:
            return {"ok": False, "error": f"Saver HTTP {r.status_code}", "code": r.status_code}
        data = r.json()
        return {"ok": True, "source": "memory_api", **data}
    except Exception as e:
...
def chat():
    ...
    # Allow Editor "Actions" commands through the single /chat endpoint
    if msg.startswith("operation="):
        kv = parse_kv_command(msg)
        op = (kv.get("operation") or "").lower()
        if op == "get_memory":
            res = proxy_get_memory(kv)
            return jsonify(res), (200 if res.get("ok") else 502)
        if op == "save_memory":
            res = proxy_save_memory(kv)
            return jsonify(res), (200 if res.get("ok") else 502)
        return jsonify({"ok": False, "error": f"Unknown operation '{op}'"}), 400

    # Persist user message immediately (so it’s available next turn)
    ...
    # Save assistant reply and any proposed memory shards
